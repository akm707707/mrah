...
// Make Modal Fixed and Vertically Centered	/ jQuery version (Bootstrap 4 default)
$('.modal').on('shown.bs.modal', function () {	
	console.log("Make Modal Fixed and Vertically Centered");
	// alert("Make Modal Fixed and Vertically Centered");
  const $modalDialog = $(this).find('.modal-dialog');
  const windowHeight  = $(window).height();
  const dialogHeight  = $modalDialog.outerHeight(true);
  const targetMargin  = Math.max(0, (windowHeight - dialogHeight) / 2);

  // Get current margin (in case something else set it)
  const currentMargin = parseFloat($modalDialog.css('margin-top')) || 0;

  if (Math.abs(currentMargin - targetMargin) < 1) return; // already there, skip
  ...
  function animate(time) {
    ...
    const newMargin = currentMargin + (targetMargin - currentMargin) * progress;
...
// Optional: reset when hidden (good practice)
