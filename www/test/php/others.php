					...
					if(@mysqli_num_rows($record) < 1){
						...
						while($purchaseinfo = mysqli_fetch_array($record, MYSQLI_ASSOC)){
							...
							$purchasequantity = $purchaseinfo['quantity'];
								$quantityarr = explode(" ", $purchasequantity);  		// convert to array
								$quantityarr[1] = arabic($quantityarr[1]);
								$purchasequantity = implode(" ",$quantityarr);			// convert to string
							...
							$purchasedescription  = $purchaseinfo['remarks'];
						...
						$log .= 'تم حذف مشتريات مواد اخرى صتف '.arabic($purchasetype);
						$log .= ' بوصف '.$purchasedescription;
						$log .= ' وكمية '.$purchasequantity;
						// $log .= ' '.arabic($unit);
						...
						*/
							...
							// $delete =  true;
							if (isset($delete)) {
								$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`title`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','purchase','حذف مشتريات مواد اخرى','$log','$time' )");
								/*
								...
								if (isset($update)) {
									$logins2 = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','water','$updatelog','$time' )");
								*/
...
//// END Of Fetch Feed
				...
				if (!isset($_POST['id']) || empty($_POST['id']) )   {
					...
					if(@mysqli_num_rows($record) < 1){
						...
						if (isset($delete)) {	
							...
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`title`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','supplier','حذف بائع مواد اخرى','$log','$time' )");
						} else {
				...
				if (!isset($_POST['id']) || empty($_POST['id']) )   {
					$responseArray = array('id' => 'danger', 'message' => 'معرف المشترى مطلوب');
					...
					if(@mysqli_num_rows($record) > 0){
						...
						$log .= 'تم تحديث حالة الدفع لمشترى '.arabic($existingtype);
						$log .= ' بوصف '.$existingremarks;
						$log .= ' بمعرف رقم '.$id;
						...
						if ($update) {	
							...
							///// End of add supplier								
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`title`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','purchase','تحديث حالة الدفع لمشتريات','$log','$time' )");
						} else {
							$responseArray = array('id' => 'danger', 'message' => 'خطأ في قاعدة البيانات $update faild');
						}
				...
				if (!isset($_SESSION['mrahid']) || empty($_SESSION['mrahid']) )   {
					...
					$responseArray = array('id' => 'danger', 'message' => 'وصف الماده مطلوب');
					...
					$responseArray = array('id' => 'danger', 'message' => 'اختيار الوحده مطلوب');
				} elseif (!isset($_POST['totalcost']) || empty($_POST['totalcost']) )   {
					...
					$description = mysqli_real_escape_string($link, $_POST['remarks']);
					$quantity = mysqli_real_escape_string($link, $_POST['quantity']);
					...
					$unitcost = $totalcost / $quantity;							$unitcost = round($unitcost, 3);   

					$quantityandunit = $quantity.' '.$unit;
					$log .= 'تم إضافة مشتريات '.arabic($type).' جديده';
					...
					*/
					...
					// if ( isset($ins1) || isset($update) ) {	
						...
						$ins2 = mysqli_query($link,"INSERT INTO `purchases`( `id`,`userid`,`mrahid`,`category`,`type`,`quantity`,`unitcost`,`totalcost`,`settlement`,`paid`,`remarks`,`suppliername`,`suppliermobile`,`time` )VALUES(	NULL,'$userid','$mrahid','others','$type','$quantityandunit','$unitcost','$totalcost','$settlement','$paid','$description','$suppliername','$suppliermobile','$time' )");			
						...
						if ($ins2) {	
							...
							if ( $suppliername !== ''  ) {
								...
								if(@mysqli_num_rows($record) > 0){									
									...
									$supins = mysqli_query($link,"INSERT INTO `suppliers`( `id`,`userid`,`mrahid`,`category`,`suppliername`,`suppliermobile`,`total`,`paid`,`time` )VALUES(	NULL,'$userid','$mrahid','others','$suppliername','$suppliermobile','$totalcost','$paid','$time' )");	
									$suplog .= 'تم إضافة بائع مواد اخزى بإسم '.$suppliername;
							...
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`title`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','purchase','إضافة مشتريات مواد اخرى','$log','$time' )");

							// $logins2 = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','water','$updatelog','$time' )");
							
							if (isset($supins)) {	
								$logins3 = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`title`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','suppliers','إضافة بائع مواد اخرى','$suplog','$time' )");
							}
						} else {
				...
				if (!isset($_SESSION['mrahid']) || empty($_SESSION['mrahid']) )   {
					...
					$log .= 'تم إضافة بائع مواد اخرى بإسم '.$suppliername;
					...
					if(@mysqli_num_rows($record) > 0){
						...
						$supins = mysqli_query($link,"INSERT INTO `suppliers`( `id`,`userid`,`mrahid`,`category`,`suppliername`,`suppliermobile`,`total`,`paid`,`time` )VALUES(	NULL,'$userid','$mrahid','others','$suppliername','$suppliermobile',0,0,'$time' )");	
						if ($supins) {	
							$responseArray = array('id' => 'success', 'message' => 'تمت الإضافة بنجاح' );
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`userid`,`mrahid`,`category`,`title`,`details`,`time` )VALUES( NULL,'$userid','$mrahid','supplier','إضافة بائع مواد اخرى','$log','$time' )");
				...
				if(isset($_POST['id']) && !empty($_POST['id']))	{	
					...
					$record = mysqli_query($link,"SELECT * FROM `purchases` WHERE userid = '$userid' AND mrahid = '$mrahid' AND category = 'others' ORDER BY time DESC    ");
					...
					if(@mysqli_num_rows($record) > 0){									
						...
						while($purchaseinfo = mysqli_fetch_array($record, MYSQLI_ASSOC)){
							...
							// convert to array to translate unit then rejoin.
								$quantityarr = explode(" ", $quantity);  		// convert to array
							$unit = $quantityarr[1];
								$quantityarr[1] = arabic($quantityarr[1]);
							$unitar = $quantityarr[1];
								$quantity = implode(" ",$quantityarr);			// convert to string
							...
							${'purchaseinfo'.$z} = array('id' => $id, 'type' => $type , 'name' => $name ,'quantity' => $quantity ,'unitcost' => $unitcost ,'totalcost' => $totalcost ,'settlement' => $settlement ,'paid' => $paid ,'remarks' => $remarks ,'suppliername' => $suppliername ,'suppliermobile' => $suppliermobile ,'time' => $time ,'unit' => $unit ,'unitar' => $unitar );	
							$z++;
						}
					}
					
					$record = mysqli_query($link,"SELECT * FROM `suppliers` WHERE userid = '$userid' AND mrahid = '$mrahid' AND category = 'others' ORDER BY time DESC    ");
					...
					$responseArray = array('id' => 'success' , 'suppliernum' => $suppliernum, 'purchasenum' => $purchasenum );
			}
		}
	}
} else {
