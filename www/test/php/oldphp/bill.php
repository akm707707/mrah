...
if($_POST){
	...
	if ( !$_SESSION ) {
		...
		if ( !isset($_SESSION['loggedin']) || $_SESSION["loggedin"] !== true )   {
			...
			if ( isset($_POST['key']) && !empty($_POST['key']) && $_POST['key'] == 'initial' ) {					
			// Fetch customers
				...
				if(@mysqli_num_rows($record) > 0){
					...
					while($customersinfo = mysqli_fetch_array($record, MYSQLI_ASSOC)){
						...
						if ($mobile == '') { $mobile = ' '; }; // to have empty values for dropdown
			...
			// Fetch Inventory Products
				...
				if(@mysqli_num_rows($record) > 0){
					...
					while($inventorysinfo = mysqli_fetch_array($record, MYSQLI_ASSOC)){
						...
						$wsbc = $inventorysinfo['wsbc'];	$wsbc = explode(",",$wsbc);
						// store BC to fetch names
						$bcArray1[] = $bc;
						// $bcArray1[] = $wsbc;
						$bcArray1 = array_merge($bcArray1,$wsbc);
					...
					for ($a = 0; $a < count($bcArray1); $a++) {
						...
						if(@mysqli_num_rows($record) > 0){					
							while($barcode = mysqli_fetch_array($record, MYSQLI_ASSOC)){
								$rbc = $barcode['barcode'];
								$rdescription = $barcode['description'];
							}
							${'barcodes'.$a} = array('bc' => $rbc,'description' => $rdescription );
						} else {
							// $record2 = mysqli_query($link,"SELECT * FROM `ownbcdb` WHERE `wsbc` = $bcArray1[$a] ");
							$record2 = mysqli_query($link,"SELECT * FROM `ownbcdb` WHERE FIND_IN_SET('$bcArray1[$a]', wsbc)");
							if(@mysqli_num_rows($record2) > 0){					
								while($barcode = mysqli_fetch_array($record2, MYSQLI_ASSOC)){
									...
									for($i=0;$i<count((array)$wsbc);$i++){
										if (  $wsbc[$i] == $bcArray1[$a] ) {
											$dbwsbc = $wsbc[$i];
											$dbwsdescription = $wsdesc[$i];
											${'barcodes'.$a} = array('bc' => $dbwsbc,'description' => $dbwsdescription );
										}
									}
								}
							}
		...
		*/
				...
				$finder = mysqli_query($link,"SELECT * FROM `cbills` ORDER BY `id` DESC LIMIT 1");
				...
				$responseArray = array('id' => 'success', 'customernum' => $customernum, 'billid' => $billid, 'Hdate' => HijriDate($hijdev), 'Gdate' => $Gdate);
			}
		}
	}
} else {
