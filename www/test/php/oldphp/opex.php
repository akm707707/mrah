	...
	if ( !$_SESSION ) {
		...
		if ( !isset($_SESSION['loggedin']) || $_SESSION["loggedin"] !== true )   {
			...
			if(isset($_POST['key']) && !empty($_POST['key']) && $_POST['key'] == 'delete' )   {
				...
				if(!isset($_POST['id']) || empty($_POST['id']) )   {
					...
					if ($cycle == 'monthly') {	$next = date('Y-m-d', strtotime('+1 month', strtotime($cycledate))); $arcycle = 'شهري'; }
					if ($cycle == 'yearly') {	$next = date('Y-m-d', strtotime('+1 year', strtotime($cycledate))); $arcycle = 'سنوي'; }
					...
					if(@mysqli_num_rows($record) > 0){
						...
						if ($update) {
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','opex','تم تحديث $logdesc بقيمة $logcost بشكل $logcycle وتبدأ في $logstart إلى $description  بقيمة $cost بشكل $arcycle وتبدأ في $cycledateformatted','$time' )");
		...
		///////////////////////////////////////////////////////////////////////////////////////
					...
					if(@mysqli_num_rows($record) > 0){
						...
						if ($ins) {
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','opex','تم إضافة $description بقيمة $cost بشكل $logcycle وتبدأ في $cycledateformatted','$time' )");
