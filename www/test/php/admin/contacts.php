		...
		// } elseif ( !str_contains($_SESSION["userclearance"], 'setting')  )   {
			...
			if(isset($_POST['fetch']) && !empty($_POST['fetch']) )   {
				...
				if(@mysqli_num_rows($record) > 0){
					while($contacts = mysqli_fetch_array($record, MYSQLI_ASSOC)){
						...
						$responseArray = array('id' => 'warning', 'message' => 'no data' );		array_push($allresps,$responseArray);
				}
			...
			*/
				...
				$logrecord = mysqli_query($link,"SELECT * FROM `contacts` WHERE `identifier` = $identifier ");
				while($log = mysqli_fetch_array($logrecord, MYSQLI_ASSOC)){
					$logaddress = $log['address'];
					$logwebsite = $log['website'];
					$logemail = $log['email'];
					$logmobile = $log['mobile'];
					$loglandline1 = $log['landline1'];
					$loglandline2 = $log['landline2'];
					$logfax = $log['fax'];
					$logtwitter = $log['twitter'];
					$logfacebook = $log['facebook'];
					$loginstagram = $log['instagram'];
					$logsnapchat = $log['snapchat'];
