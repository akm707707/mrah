	...
	if ( !$_SESSION ) {
		...
		// } elseif ( !str_contains($_SESSION["userclearance"], 'setting')  )   {
			...
			if(isset($_POST['fetch']) && !empty($_POST['fetch']) )   {
				...
				$entity = array('id' => $id, 'Aname' => $Aname, 'crid' => $crid, 'taxid' => $taxid, 'vat' => $vat, 'hijdev' => $hijdev, 'fiscal' => $fiscal, 'username' => $username, 'identifier' => $identifier);
				...
				if( isset($_POST['hijdev']) )   {
					if( !empty($_POST['hijdev']) && !is_numeric($_POST['hijdev']) )   {
						$responseArray = array('id' => 'danger', 'message' => 'يجب أن تحتوي تعديل السنه الهجريه على أرقام فقط');	
						array_push($allresps,$responseArray);
					} elseif ( empty($_POST['hijdev']) )   {
						$hijdev = 0;
					} else {	
						$hijdev = mysqli_real_escape_string($link, $_POST['hijdev']);
					}
					// echo $Aname;
					$record = mysqli_query($link,"SELECT * FROM `entities` WHERE id = '$identifier'   ");
					// echo mysqli_num_rows($record);
					if(@mysqli_num_rows($record) > 0){
						$record = mysqli_fetch_array($record, MYSQLI_ASSOC);
						  if ($hijdev != $record['hijdev']) {
							  $update = mysqli_query($link,"UPDATE `entities` SET `hijdev` = '$hijdev'	WHERE `id`='$identifier'");
								if ($update) {	
									$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','setting','تم تحديث تعديل السنه الهجريه من $loghijdev إلى $hijdev','$time' )");
									$responseArray = array('id' => 'success', 'message' => 'تم تحديث تعديل السنه الهجريه  بنجاح');
									array_push($allresps,$responseArray);
								} else {
									$responseArray = array('id' => 'danger', 'message' => 'حدث خطأ أثناء عملية التحديث');
									array_push($allresps,$responseArray);
							...
							$responseArray = array('id' => 'warning', 'message' => 'تعديل السنه الهجريه مطابقه للسابق'); 
							array_push($allresps,$responseArray);
						  }
					} else {
						$responseArray = array('id' => 'danger', 'message' => 'لم يتم العثور على أي بيانات');
