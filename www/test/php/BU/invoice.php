	...
	if ( !$_SESSION ) {
		...
		if ( !isset($_SESSION['loggedin']) || $_SESSION["loggedin"] !== true )   {
			...
			if(isset($_POST['costumername']) && !empty($_POST['costumername']) )   {
				$costumername = mysqli_real_escape_string($link, $_POST['costumername']);
				$record1 = mysqli_query($link,"SELECT * FROM `costumers` WHERE `name`='$costumername' ");
...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	// if ( isset($costumernum) && $costumernum > 0 ){	
		// for($a=0;$a<$costumernum;$a++){ 
			// array_push($Totalcostumers,${'costumerinfo'.$a}); 
		// }	
		// array_push($responseArray,$Totalcostumers); 
	// }
