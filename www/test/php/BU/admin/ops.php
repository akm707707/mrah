	...
	if ( !$_SESSION ) {
		...
		if ( !isset($_SESSION['loggedin']) || $_SESSION["loggedin"] !== true )   {
			...
			if(isset($_POST['key']) && !empty($_POST['key']) )   {				
				if ( $_POST['key'] == 'daily' ) { $since = $todaystartunix; }
				if ( $_POST['key'] == 'weekly' ) { $since = $thisweekstartunix; }
				if ( $_POST['key'] == 'monthly' ) { $since = $thismonthstartunix; }
				if ( $_POST['key'] == 'yearly' ) { $since = $thisyearstartunix; }
				if ( $_POST['key'] == 'all' ) { $since = '28127847'; }
				// fetch today sales
				$record = mysqli_query($link,"SELECT * FROM `invoices` WHERE timeadded >= '$since' ");
