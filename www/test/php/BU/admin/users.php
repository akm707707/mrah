		...
		// } elseif ( !str_contains($_SESSION["userclearance"], 'setting')  )   {
			...
			if(isset($_POST['key']) && !empty($_POST['key']) && $_POST['key'] == 'delete' )   {	// DELETE EMPLOYEE SECTION //////////////
				if(isset($_POST['id']) && !empty($_POST['id']) )   {
					...
					if ($delete) {		
						$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','user','تم حذف الموظف $logname  باسم مستخدم $logusername  وجوال $logmobile  وايميل $logemail  وراتب $logsalary وبدلات $logallowences وحاله $logstatus وتصريح $logclearance ','$time' )");
				...
				if (!isset($_POST['name']) || empty($_POST['name']) )   {
					...
					if(@mysqli_num_rows($record) > 0){
						...
						if ($ins) {
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','user','تم إضافة موظف بإسم $name  باسم مستخدم $mobile  وجوال $mobile  وايميل $email  وراتب $salary وبدلات $allowences وحاله $status وتصريح $clearance','$time' )");
				...
				if (!isset($_POST['name']) || empty($_POST['name']) )   {
					...
					if(@mysqli_num_rows($record) > 1){
						...
						if ($update) {
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','user','تم تحديث بيانات الموظف $logname  باسم مستخدم $logusername  وجوال $logmobile  وايميل $logemail  وراتب $logsalary وبدلات $logallowences وحاله $logstatus وتصريح $logclearance إلى اسم $name  باسم مستخدم $mobile  وجوال $mobile  وايميل $email  وراتب $salary وبدلات $allowences وحاله $status وتصريح $clearance','$time' )");
							// Handle photo upload 
							if (isset($_POST['photo']) && !empty($_POST['photo']) )   {
