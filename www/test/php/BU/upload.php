				...
				if(!isset($_POST['billtotal']) || empty($_POST['billtotal']) )   {
					...
					if ( !$_FILES ) {	// handle Images
						...
						if(!isset($_POST['original']) || empty($_POST['original']) )   {
							...
							$fileext = Fileext($_POST['original']);
							$imgins = mysqli_query($link,"INSERT INTO `pbills`( `id`, `identifier`, `ext`, `total` , `settlement` , `paid` , `paymenttype` , `timeadded` , `timeedited` )VALUES( NULL, '$identifier', '$fileext', '$billtotal', '$settlement', '$paid', '$paymenttype', '$time', NULL )");	
							...
							if ($imgins) {
								...
								$responseArray = array('id' => 'danger', 'message' => 'حدث خطأ في الخادم $imgins');
							}
						}
					} else {	// Handdle PDF
						...
						$pdfins = mysqli_query($link,"INSERT INTO `pbills`( `id`, `identifier`, `ext`, `total` , `settlement` , `paid` , `paymenttype` , `timeadded` , `timeedited` )VALUES( NULL, '$identifier', 'pdf', '$billtotal', '$settlement', '$paid', '$paymenttype', '$time', NULL )");	
						...
						if ($pdfins) {
							...
							$targetFilePath = $directory . $billid . '.pdf';
