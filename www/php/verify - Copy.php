...
session_start();
		...
		$dataarr = mysqli_query($link,"SELECT * FROM `users` WHERE `id`='1' " );
		...
		if ( mysqli_num_rows ( $dataarr ) > 0 ) {
			...
			if ( isset($_SESSION['mrahid']) && !empty($_SESSION['mrahid']) && isset($_SESSION['mrahname']) && !empty($_SESSION['mrahname']) ) {
				$data = array('userid' => $userid, 'name' => $name, 'mobile' => $mobile, 'email' => $email, 'mrah' => $mrah,'marketvalue' => $marketvalue,'time' => $timecreated,'mrahid' => $_SESSION['mrahid'],'mrahname' => $_SESSION['mrahname'],'profit' => $_SESSION['profit']);
