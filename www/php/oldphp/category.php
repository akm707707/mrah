	...
	if ( !$_SESSION ) {
		...
		if ( !isset($_SESSION['loggedin']) || $_SESSION["loggedin"] !== true )   {
			...
			if(isset($_POST['key']) && !empty($_POST['key']) && $_POST['key'] == 'delete' )   {
				if(isset($_POST['id']) && !empty($_POST['id']) )   {
					...
					if ($delete) {
						if(isset($_POST['cleancat']) && !empty($_POST['cleancat']) )   {	
							$update2 = mysqli_query($link,"UPDATE `ownbcdb` SET `category`='' WHERE `category`='$logcategory'");
							if ($update2) {	
								$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','category','تم حذف التصنيف $logcategory مع إزالته من كافة المنتجات','$time' )");
							}
						} else {
							$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','category','تم حذف التصنيف $logcategory بدون إزالته من كافة المنتجات','$time' )");
						}
				...
				if(!isset($_POST['id']) && empty($_POST['id']) )   {
					...
					$record = mysqli_query($link,"SELECT * FROM `category` WHERE category = '$categoryname' AND id != '$id' ");
					...
					if(@mysqli_num_rows($record) > 0){
						...
						if ($update) {	
							...
							if(isset($_POST['replace']) && !empty($_POST['replace']) )   {	
								$update2 = mysqli_query($link,"UPDATE `ownbcdb` SET `category`='$categoryname' WHERE `category`='$logcategory'");
								if ($update2) {	
									$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','category','تم تحديث التصنيف $logcategory إلى $categoryname مع تحديث تصنيف كافة المنتجات','$time' )");
								}
							} else {
								$logins = mysqli_query($link,"INSERT INTO `logs`( `id`,`eid`,`name`,`section`,`info`,`timeadded` )VALUES(	NULL,'$identifier','$empname','category','تم تحديث التصنيف $logcategory إلى $categoryname بدون تحديث تصنيف المنتجات','$time' )");
							}
						} else {
							$responseArray = array('id' => 'danger', 'message' => 'خطأ في قاعدة البيانات');
						}
		...
		///////////////////////////////////////////////////////////////////////////////////////
					...
					if(@mysqli_num_rows($record) > 0){
						...
						// $finder = mysqli_query($link,"SELECT * FROM `category` ORDER BY `id` DESC LIMIT 1");
